...
/** mały szkic śladu rejsu (bez mapy – działa offline) */
export function TrackSketch() {
  const len = useSyncExternalStore(subscribeTrack, () => getTrack().length);
  const pts = getTrack();
  if (len < 2) return <p className="muted small">Ślad pojawi się po wypłynięciu z portu.</p>;
  ...
  return (
    <svg className="track" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Szkic śladu rejsu">
